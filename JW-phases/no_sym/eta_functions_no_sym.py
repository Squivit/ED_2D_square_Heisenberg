from eta_lattice_no_sym import SpinConfiguration
import numpy as np
...
class SpinCorrelatorMachine:
    ...
    def get_SDSF_minus(self, get_n_lowest = 150, print_en_diff = False):
        ...
        self.sc_minus_helper.generate_hamiltonian(lamb=self.lamb, delta=self.delta)
        ...
        for ik, k in enumerate(self.k_points):
            ...
            if self.eta != 1:
                eta_phases = np.loadtxt(fr'JW-phases/no_sym/eta_phases_{self.nx}x{self.ny}_S-.txt')
                ide = 0
                        
            S_minus_k_mapped_gs = np.zeros(len(self.sc_minus_helper.spin_confg), dtype=np.complex64)

            for repr, ii in self.sc.spin_confg.items():
                ...
                for y, x in zip(*np.where(config == 1)):
                    ...
                    ind = self.sc_minus_helper.map_int_to_basis(state_flipped)

                    phase = phase_map[y, x]

                    #if self.eta != 1:
                    #    eta_angle = eta_phases[ide]
                    #    phase *= np.exp(-1j * (1. - self.eta) * eta_angle)
                    #    ide += 1
                          
                    S_minus_k_mapped_gs[ind] += coeff * phase * norm
...
def plot_SDSF(scm = None, nx = 4, ny = 4, lamb = 1, delta = 1, dir = '+-', to_diameter = False, graph_normalized = False, marker_s = 900, k_lowest = 50, save = False, eta = 1.):
    ...
    for id, delta in enumerate(deltas):
        for il, lamb in enumerate(lambdas):
            if delta != 0. or lamb != 0.:
                ...
                plot_SDSF(scm, 4, 4, lamb=lamb, delta=delta, dir='+-', eta = 0., save=False)
