...
class SpinCorrelatorMachine:
    ...
    def get_SDSF_minus(self, get_n_lowest = 35, print_en_diff = False, n_workers = None):
        ...
        repr_size = len(self.sc.representatives)
        # Split work into chunks for parallel processing
        chunk_size = max(1, repr_size // (n_workers * 4) + 1)
        ...
        for ik, k in enumerate(self.k_points):
            ...
            x_exp = np.exp(1j * np.pi * k[1] * np.arange(self.nx))
            y_exp = np.exp(1j * np.pi * k[0] * np.arange(self.ny))
            self.phase_map = np.outer(y_exp, x_exp)

            self.norm = 1. / np.sqrt(float(self.n))
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                ...
                for repr, ii in self.sc.representatives.items():
                    ...
                    if len(chunk) >= chunk_size:
                        futures.append(executor.submit(self._process_state_batch_pm, chunk.copy()))
                        chunk.clear()
                ...
                for future in as_completed(futures):
                    ...
                    self._merge_results_pm(batch_results)
            
            print(np.sum(np.square(np.abs(self.S_minus_k_mapped_gs))))
    ...
    def _process_state_batch_pm(self, states):
        ...
        for state in states:
            ...
            for y, x in zip(*np.where(config == 1)):
                ...
                phase = self.phase_map[y, x]

                # average over all translations
                phase_r_2 = np.mean(self.phase_map[tuple(np.transpose(translations))])
                        
                norm_k = np.sqrt( self.sc.norms[repr] / self.sc_minus_helper.norms[repr_f] )
            
            results.append({
                'ind': ind,
                'coeff': coeff * phase * phase_r_2 * self.norm * norm_k,
            })
            
        return results
    
    def _merge_results_pm(self, batch_results):
...
def plot_SDSF(scm = None, nx = 4, ny = 4, lamb = 1, delta = 1, dir = '+-', to_diameter = False, graph_normalized = False, marker_s = 900, k_lowest = 150, save = False, eta = 1.):
    ...
    for id, delta in enumerate(deltas):
        for il, lamb in enumerate(lambdas):
            if delta != 0. or lamb != 0.:
                ...
                plot_SDSF(scm, 6, 4, lamb=lamb, delta=delta, dir='+-', eta = 0., save=False)
