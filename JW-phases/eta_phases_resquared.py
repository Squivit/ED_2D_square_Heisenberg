...
class Representation():
    ...
    def get_eta_phases(self):
        ...
        x_index = np.arange(0, (2*k + 1) * n, 1) - x0
        y_index = np.arange(0, (2*k + 1) * n, 1) - y0
        ...
        def extended_phase(_rot):
            if nx > ny:
                rot = np.zeros( (_rot.shape[0] + nx - ny, _rot.shape[1]) )
                rot[0, :] = _rot[-1, :]
                rot[1:-1, :] = _rot
                rot[-1, :] = _rot[0, :]

            config_extended = np.concatenate((2*k + 1)*[rot], axis = 1)
            config_extended = np.concatenate((2*k + 1)*[config_extended], axis = 0)
        ...
        np.savetxt(fr'JW-phases/eta_phases_{n}x{self.n_y}_Sz={int(n*self.n_y/2-self.magnon_number)}.txt', eta_phases)
