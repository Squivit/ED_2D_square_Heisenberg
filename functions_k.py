...
class SpinCorrelatorMachine:
    ...
    def __init__(self, _system_size = (2, 2), mag = -1, _delta : float = 1., _lamb : float = 1., _J2overJ1 = 1., ks = [[0, 0], [0, .5]]):
        ...
        self.sc = SpinConfiguration(self.nx, self.ny, magnon_number=self.mag, print_data=False, delta = self.delta,
                                    J2overJ1=self.J2, lamb=self.lamb, lowest_eignstates=1)
    ...
    def get_SDSF_minus(self, get_n_lowest = 35, print_en_diff = False):
        ...
        if self.sc_minus_helper is None:
            ...
            self.sc_minus_helper = SpinConfiguration(self.nx, self.ny, self.mag - 1, self.delta, self.lamb, self.J2,
...
def plot_SDSF(scm = None, nx = 4, ny = 4, lamb = 1, delta = 1, dir = '+-',
        ...
        for il, lamb in enumerate(lambdas):
            if delta != 0. or lamb != 0.:
                ...
                plot_SDSF(scm, 4, 6, lamb=lamb, delta=delta, dir='+-', marker_s=marker_size, graph_normalized=graph_normalized,
                          to_diameter=to_diameter, save=True, k_lowest=20)
