...
def get_SDSF(scm = None, key = '16B', lamb = 1, delta = 1, dir = '+-', plot = False, print_data = False, to_diameter = False, graph_normalized = False, marker_s = 20, k_lowest = 150, save = True):
...
def get_lambda_var(key = '18A', filename = '', delta = 1., n_low = 25, also_zz = True):
    ...
    if also_zz:
        for var in variables:
            start_time = time.time()
            scm = get_SDSF(scm, key, lamb=var, delta=delta, dir='zz', plot=False, print_data = False, k_lowest=n_low)
            work_time = time.time() - start_time
