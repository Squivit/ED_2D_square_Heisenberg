...
class SpinConfiguration:
    ...
    def get_hamiltonian(self, k = None, lamb = None, delta = None, force_ham_gen = False):
        ...
        k_formatted = ', '.join([str(kf) for kf in np.round(k, 3)]).replace('.', '_')

        # there is a correct hamiltonian saved
        if hamilonians.__contains__(f'{self.key}_k={k_formatted}_d={self.delta}_l={self.lamb}_Sz={abs(int(self.n/2) - self.magnon_number)}.npz'):
            self.hamiltonian = load_npz(fr'{HAMILTONIAN_PATH}/{self.key}_k={k_formatted}_d={self.delta}_l={self.lamb}_Sz={abs(int(self.n/2) - self.magnon_number)}.npz')
            if self.print:
                print('Hamiltonian loaded successfully.')

        # check if there is a hamiltonian with D=1, l=1
        elif hamilonians.__contains__(f'{self.key}_k={k_formatted}_d=1.0_l=1.0_Sz={abs(int(self.n/2) - self.magnon_number)}.npz'):

            from clusters_param_change import Parameter_Changer
            pc = Parameter_Changer(key=self.key)
            ham_1_1 = load_npz(fr'{HAMILTONIAN_PATH}/{self.key}_k={k_formatted}_d=1.0_l=1.0_Sz={abs(int(self.n/2) - self.magnon_number)}.npz')
    ...
    def _process_state_batch(self, states):
        ...
        for state_int in states:
            ...
            for w, _, inv in self.weight_matrices:
    ...
    def save_hamiltonian(self):
        ...
        k_formatted = ', '.join([str(kf) for kf in np.round(self.k, 3)]).replace('.', '_')

        fname = fr'{HAMILTONIAN_PATH}/{self.key}_k={k_formatted}_d={self.delta}_l={self.lamb}_Sz={abs(int(self.n/2) - self.magnon_number)}.npz'
